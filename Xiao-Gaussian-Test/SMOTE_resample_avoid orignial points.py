...
k = 5  #! 后面生成test集的预测值比较分成几副小图
K_num_list = [10] #! 高斯模型遍历parameter K-fold cross validation 的parameter（不过现在采用的是log marginial likelihood，所以不需要设置这个，随便取一个数字就行）
length_scale_list = [1] #! 高斯模型遍历parameter
noise_level = 0.01 #! noise level for the gaussian process model(0.1 means standard deviation)
...
plt.hist(sigma, bins=30, edgecolor='black')
plt.xlabel('Sigma')
plt.ylabel('Frequency')
